...
fabric.enableGLFiltering = false; // image resize error
...
class Graphics {
  ...
  toDataURL(type) {
    let ret = '';
    if (this._canvas) {
      let bg = this._canvas.backgroundColor;
      this._canvas.backgroundColor = '#fff';
      ret = this._canvas.toDataURL(type);
      this._canvas.backgroundColor = bg;
    }
   ...
   * @param {float} value - w: scale with width, h: scale with height, other: ratio
   * @memberof Graphics
   */
  adjustCanvasDimension(value) {
    const canvasImage = this.canvasImage.scale(1);
    const { width, height } = canvasImage.getBoundingRect();
    const maxDimension = this._calcMaxDimension(width, height);

    if (!value) {
      if (this.ratio) value = this.ratio;
      else value = 'h';
    }

    let ratio = value;
    if (value === 'w' || value === 'h') {
      const parent = this._canvas.getElement().parentNode.parentNode;
      const cw = parent.clientWidth;
      const ch = parent.clientHeight;

      if (value === 'w') ratio = cw / width;
      else if (value === 'h') ratio = ch / height;
    }
    this.ratio = ratio;

    let w = Math.round(ratio * width);
    let h = Math.round(ratio * height);

    if (w > maxDimension.width || h > maxDimension.height) {
      maxDimension.width = w;
      maxDimension.height = h;
    }
   ...
   */
  _onPathCreated(obj) {
    // do this will change the path position
