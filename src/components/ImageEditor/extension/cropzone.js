...
const Cropzone = fabric.util.createClass(
  ...
  /** @lends Cropzone.prototype */ {
    ...
    _render(ctx) {
      ...
      // Render outer rect
      this._fillOuterRect(ctx, "rgba(185, 244, 100, 0.22)");
    ...
    _fillOuterRect(ctx, fillStyle) {
      ...
      // console.log(`nX=> 0:${x[0]} 1:${x[1]} 2:${x[2]} 3:${x[3]}`);
      // console.log(`nY=> 0:${y[0]} 1:${y[1]} 2:${y[2]} 3:${y[3]}`);
    ...
    _getCoordinates(ctx) {
      const width = this.getScaledWidth(),
        ...
        left = this.left,
        top = this.top,
        canvasEl = this.canvas; // canvas element, not fabric object

      // const log = `Get=> left:${left} top:${top} width:${width} height:${height}`;
      // console.log(log);
      return {
        x: snippet.map(
          [
